...
def get_best_sequence(ctc_output, language_models, c2p, p2c, true_transcription, K, CTC_weight, LM_weight):
    ...
    lm14 = language_models['model_14gram']
    lm15 = language_models['model_15gram']
    correct_sequence = true_transcription
    """This seems to be close, but it's not right so check it thoroughly later"""

    """Let's find the CTC decoding sequence for debugging"""
    output_classes = np.argmax(ctc_output, axis=1)
    ...
    predicted_phones_ctc = [c2p[x] for x in blanks_eliminated]
    ...
    for i, frame in enumerate(non_blank_frames):
        if i == 1:
            ...
            for class_value, logprob in enumerate(frame):
                if class_value > 0:  # ignore the blank symbol
                    ...
                    elif len(evaluation_seq) == 14:
                        context = (evaluation_seq[0], evaluation_seq[1], evaluation_seq[2],
                                   evaluation_seq[3], evaluation_seq[4], evaluation_seq[5],
                                   evaluation_seq[6], evaluation_seq[7], evaluation_seq[8],
                                   evaluation_seq[9], evaluation_seq[10], evaluation_seq[11],
                                   evaluation_seq[12])
                        lm_score = lm14.score(word=evaluation_seq[13], context=context)
                    elif len(evaluation_seq) == 15:
                        context = (evaluation_seq[0], evaluation_seq[1], evaluation_seq[2],
                                   evaluation_seq[3], evaluation_seq[4], evaluation_seq[5],
                                   evaluation_seq[6], evaluation_seq[7], evaluation_seq[8],
                                   evaluation_seq[9], evaluation_seq[10], evaluation_seq[11],
                                   evaluation_seq[12], evaluation_seq[13])
                        lm_score = lm15.score(word=evaluation_seq[14], context=context)
                    ...
                    CTC_score = ALPHA*logprob
                    if math.isnan(total_score):
                        stop = None
                    data = {'seq': tuple(seq), 'score': total_score, 'CTC_score': CTC_score}
        ...
        if i > 1:
            ...
            for seq_data in beam_sequences:
                seq = seq_data['seq']
                accumulated_prob = seq_data['CTC_score']  # This is critical! Only cumulative CTC scores propagate
                possible_sequences = []
                for class_value, logprob in enumerate(frame):
                    if class_value > 0:  # ignore the blank symbol
                        symbol = c2p[class_value]
                        ...
                        try:
                            ...
                            lm_score = -1000000
                        total_score = ALPHA * (logprob + accumulated_prob) + BETA * lm_score
                        CTC_score = ALPHA * (logprob + accumulated_prob)
                        data = {'seq': tuple(newseq), 'score': total_score, 'CTC_score': CTC_score}
...
def get_language_model():
    ...
    return {'model_1gram': model1, 'model_2gram': model2, 'model_3gram': model3,
            ...
            'model_10gram': model10, 'model_11gram': model11, 'model_12gram': model12,
            'model_13gram': model13, 'model_14gram': model14, 'model_15gram': model15}
...
def main():
    """"""
    models = get_language_model()
    incorrect = joblib.load('./exps/PARTITION_2_trial_1_Oracle_Baseline_CTC_training/ctc_output_predictions_test/M05_B2_CW73_M7.pkl')
    stop = 'None'
    phones = joblib.load('phones.pkl')
    p2c = utils.phone2class(phones)
    c2p = utils.class2phone(phones)
    ...
    _ = get_best_sequence(ctc_output=incorrect['ctc_outputs'], language_models=models, c2p=c2p, p2c=p2c,
                      true_transcription=incorrect['true_phones'], K=K, CTC_weight=CTC_weight, LM_weight=LM_weight)
